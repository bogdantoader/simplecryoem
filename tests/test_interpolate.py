...
class TestInterpolate(unittest.TestCase):
    ...
    def test_find_nearest_one_grid_point_idx_bug(self):
        coord = jnp.array([0.282842712e00, 2.22044605e-16, 0.00000000e00])
        x_freq = jnp.array([0.0, 0.1, 0.2, -0.2, -0.1])
        y_freq = x_freq
        z_freq = x_freq
    ...
    def test_interpolate_nn(self):
        ...
        i_coords = jnp.array(
            [[0, 0.04, -0.24, -0.01], [0, 0, -2, 4], [0, 0, 0.168, -0.48]]
        )

        i_vol_correct = jnp.array(
            [vol[0, 0, 0], vol[0, 1, 0], vol[4, 5, 1], vol[2, 0, 3]]
        )
    ...
    def test_interpolate_tri(self):
        ...
        assert_array_equal(
            interpolate_diff_grids(i_coords, x_grid, y_grid, z_grid, vol, "tri"), vals
        )
    ...
    def test_find_adjacent_grid_points_idx(self):
        ...
        ) = self.get_data_adjacent_and_nearest_grid_point_idx_odd()
        ...
        (
            grid,
            pts_adjacent_nearest,
        ) = self.get_data_adjacent_and_nearest_grid_point_idx_even()
    ...
    def get_data_adjacent_and_nearest_grid_point_idx_even(self):
        grid = jnp.array([0.0, 0.25, -0.5, -0.25])
