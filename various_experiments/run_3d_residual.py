...
from simplecryoem.noise import estimate_noise, average_radially
...
def parse_args(parser):
    ...
    parser.add_argument(
        "star_file_proj",
        help="Name of the star file for the projections, relative to data_dir.",
    )
    parser.add_argument(
        "star_file_img",
        help="Name of the star file for the images, relative to data_dir.",
    )
    ...
    parser.add_argument(
        ...
        "--N_px_noise",
        type=int,
        help="Length of the corner side to use for noise estimation.",
    )
    parser.add_argument(
        "-Nin",
        "--N_imgs_noise",
    ...
    parser.add_argument(
        "-r", "--radius", type=float, help="Mask radius.", default=np.inf
    )
    parser.add_argument(
        "-Nb",
        "--N_batches",
        type=int,
        help="Number of batches to process the coordinates on the GPU.",
        default=600,
    )
    parser.add_argument(
        "-sp",
        "--spatial",
        help="If this flag is used, don't take the Fourier transofrm of the images.",
        action="store_true",
    )
    parser.add_argument(
        "-sig",
        "--sigma_noise",
        help="If this flag is used, estimate sigma_noise and store in a separate volume.",
        action="store_true",
    )
...
def main(args):
    ...
    _, imgs = load_data(
        args.data_dir, args.star_file_proj, load_imgs=True, fourier=False
    )
    params0, imgs0 = load_data(
        args.data_dir, args.star_file_img, load_imgs=True, fourier=False
    )
    ...
    if args.sigma_noise:
        ...
        t0 = time.time()
        sigma_noise_estimated = estimate_noise(
            imgs_noisy[:N_imgs_noise], nx_empty=N_px_noise, nx_final=nx
        ).reshape([nx, nx])
    ...
    vol, _, _ = get_volume_residual(
        imgs - imgs0, angles, sigma_noise, x_grid, args.radius, args.N_batches
    )
    vol0, vol_sigma, vol_counts = get_volume_residual(
        imgs0, angles, sigma_noise, x_grid, args.radius, args.N_batches
    )

    # And print to file
    with mrcfile.new(
        f"{args.out_dir}/{args.out_file}_{nx}_resid.mrc", overwrite=True
    ) as mrc:
        mrc.set_data(jnp.fft.fftshift(vol).astype(np.float32))

    with mrcfile.new(
        f"{args.out_dir}/{args.out_file}_{nx}_imgs.mrc", overwrite=True
    ) as mrc:
        mrc.set_data(jnp.fft.fftshift(vol0).astype(np.float32))

    with mrcfile.new(
        f"{args.out_dir}/{args.out_file}_{nx}_counts.mrc", overwrite=True
    ) as mrc:
        mrc.set_data(jnp.fft.fftshift(vol_counts).astype(np.float32))

    if args.sigma_noise:
        with mrcfile.new(
            f"{args.out_dir}/{args.out_file}_{nx}_sigma.mrc", overwrite=True
        ) as mrc:
