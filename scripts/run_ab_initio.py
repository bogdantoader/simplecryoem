...
def parse_args(parser):
    ...
    parser.add_argument(
        ...
        "--N_batch",
        type=int,
        help="Number of batches for calculating the loss and gradient.",
        default=1,
    )
    parser.add_argument(
        "-Ni", "--N_iter", type=int, help="Number of iterations.", default=10000000
    )
    parser.add_argument(
        "-r0", "--radius0", type=float, help="Starting radius for frequency marching."
    )
    parser.add_argument(
        "-a", "--alpha", type=float, help="Regularisation parameter.", default=1e-9
    )
    parser.add_argument(
        "-sgdbs",
        "--sgd_batch_size",
        type=int,
        help="Batch size for initialisation SGD run.",
        default=300,
    )
    parser.add_argument(
        "-sgdlr",
        "--sgd_learning_rate",
        type=float,
        help="Learning rate for initialisation SGD run.",
        default=1e6,
    )
    parser.add_argument(
        "-ei",
        "--eps_init",
        type=float,
        help="Stopping criterion epsilon for initialisation SGD run.",
        default=2e-7,
    )
    parser.add_argument(
        "-Nsv",
        "--N_samples_vol",
        type=int,
        help="Number of MCMC samples of the volume at each iteration.",
        default=101,
    )
    parser.add_argument(
        "-Nsag",
        ...
        type=int,
        help="Number of global MCMC samples of the orientations at each iteration.",
        default=1001,
    )
    parser.add_argument(
        "-Nsal",
        ...
        type=int,
        help="Number of local MCMC samples of the orientations at each iteration.",
        default=201,
    )
    parser.add_argument(
        "-Nss",
        "--N_samples_shifts",
        type=int,
        help="Number of MCMC samples of the shifts at each iteration.",
        default=101,
    )
    parser.add_argument(
        "-L",
        "--L_hmc",
        type=int,
        help="Number of step sizes for HMC integration.",
        default=5,
    )
    parser.add_argument(
        "-FM",
        "--freq_marching_steps",
        type=int,
        help="Number of iterations before increasing the Fourier radius.",
        default=8,
    )
    parser.add_argument(
        "-Mbs",
        "--minibatch_factor",
        type=int,
        help="Size of minibatch to work with at each iteration is nx_iter * minibatch_factor",
        default=50,
    )
