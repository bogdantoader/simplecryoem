...
def ab_initio_mcmc(
    ...
    if vol0 is None and opt_vol_first:
        ...
        v, angles, shifts, z = initialize_ab_initio_vol(
            key,
            project_func,
            rotate_and_interpolate_func,
            apply_shifts_and_ctf_func,
    ...
    for idx_iter in range(1, N_iter + 1):
        ...
        if recompile:
            ...
            (
                ...
                grad_loss_volume_sum_iter,
                grad_loss_volume_sum_z_iter,
                loss_func_angles,
                loss_func_batched0_iter,
                loss_func_sum_iter,
                loss_func_sum_z_iter,
                loss_func_batched_z_iter,
                loss_proj_func_batched0_iter,
                rotate_and_interpolate_iter,
            ) = get_jax_ops_iter(
                project_func,
                rotate_and_interpolate_func,
                apply_shifts_and_ctf_func,
            ...
            (
                proposal_func_orientations_unif,
                proposal_func_orientations_pert,
            ...
            (
                proposal_z,
                proposal_angles_z,
                proposal_angles_z_mtm,
            ) = get_class_proposal_func(
                loss_func_batched_z_iter,
                loss_func_sum_z_iter,
                sigma_noise_iter,
                alpha_d,
        ...
        if angles0 is None:
            ...
            if idx_iter < 64 or jnp.mod(idx_iter, 8) == 4:
                ...
                for i in jnp.arange(N1):
                    ...
                    if z0 is None:
                        ...
                        params_orientations_z = {
                            "v": v,
                            "shifts": shifts[i],
                            "ctf_params": ctf_params[i],
                            "imgs": imgs_iter[i],
                        }

                        az0 = jnp.concatenate([angles[i], z], axis=1)
                        _, r_samples_angles, samples_az = mcmc(
                            key_angles,
                            proposal_angles_z_mtm,
                            az0,
                        ...
                        angles_new.append(
                            samples_az[N_samples_angles_global - 2, :, :3]
                        )
                        z = (
                            samples_az[N_samples_angles_global - 2, :, [3]]
                            .transpose()
                ...
                if verbose:
                    ...
                    print(
                        f"  mean(a_angles) = {jnp.mean(r_samples_angles[~jnp.isnan(r_samples_angles)])} ({jnp.sum(jnp.isnan(r_samples_angles))} nans)",
                        flush=True,
                    )
                    print(
                        f"  max(a_angles) = {jnp.max(r_samples_angles)}, min(a_angles) = {jnp.min(r_samples_angles)}"
                    )
            ...
            if idx_iter >= 64:
                ...
                if verbose:
                    ...
                    print(
                        f"  mean(a_angles) = {jnp.mean(r_samples_angles)}", flush=True
                    )
        ...
        if shifts0 is None:
            ...
            proj = jnp.array(
                [
                    rotate_and_interpolate_iter(v, angles[i])
                    for i in jnp.arange(angles.shape[0])
                ]
            )
        ...
        for k in jnp.arange(K):
            ...
            if N1 == 1:
                params_vol = {
                    "angles": angles[0, zkidx],
                    "shifts": shifts[0, zkidx],
                    "ctf_params": ctf_params[0, zkidx],
                    "imgs_iter": imgs_iter[0, zkidx],
                }
            else:
                params_vol = {
                    "angles": angles[:, zkidx],
                    "shifts": shiftsl[:, zkidx],
                    "ctf_params": ctf_params[:, zkidx],
                    "imgs_iter": imgs_iter[:, zkidx],
                }
        ...
        if jnp.mod(idx_iter, 1) == 0 and save_to_file:
            for class_idx in jnp.arange(K):
                with mrcfile.new(
                    f"{out_dir}/rec_iter_{idx_iter}_class{class_idx}.mrc",
                    overwrite=True,
                ) as mrc:
    ...
    if save_to_file:
        for class_idx in jnp.arange(K):
            vr = jnp.real(jnp.fft.fftshift(jnp.fft.ifftn(v[class_idx])))
            with mrcfile.new(
                f"{out_dir}/rec_final_class{class_idx}.mrc", overwrite=True
            ) as mrc:
                mrc.set_data(vr.astype(np.float32))
...
def initialize_ab_initio_vol(
    ...
    sgd_grad_func = get_sgd_vol_ops(
        grad_loss_volume_sum, angles[0], shifts[0], ctf_params[0], imgs[0], sigma_noise
    )
    v = sgd(
        sgd_grad_func,
        N2,
    ...
    if K > 1:
        ...
        v += jnp.array(
            np.random.randn(K, nx, nx, nx) + np.random.randn(K, nx, nx, nx) * 1j
        )
...
def get_diagnostics_funs_iter(project_func, x_grid, mask, alpha=0, interp_method="tri"):
    slice_func, slice_func_array, slice_func_array_angles = get_slice_funcs(
        project_func, x_grid, mask, interp_method
    )
    loss_func, loss_func_batched, loss_func_sum = get_loss_funcs(
...
def get_jax_ops_iter(
    ...
    loss_proj_func_batched0 = get_loss_proj_funcs(
        apply_shifts_and_ctf_func, x_grid, alpha=0
    )

    @jax.jit
    def rotate_and_interpolate(v, angles):
        return jax.vmap(rotate_and_interpolate_func, in_axes=(None, 0, None, None))(
            v * mask, angles, x_grid, x_grid
        )

    return (
        slice_func_array_angles,
        grad_loss_volume_sum,
        grad_loss_volume_sum_z,
        loss_func_angles,
        loss_func_batched0,
...
def get_jax_proposal_funcs(
    ...
    def proposal_func_orientations_batch(key, angles0, logPiX0, v, shifts):
        ...
        def logPi(a):
            a_i = [
                -loss_func_batched0_iter(
                    v, a[i], shifts[i], ctf_params[i], imgs_iter[i], sigma_noise_iter
                )
                for i in jnp.arange(angles0.shape[0])
            ]
    ...
    def proposal_func_orientations(
        key,
        angles0,
        logPiX0,
        v,
        ...
        logPi = lambda a: -loss_func_batched0_iter(
            v, a, shifts, ctf_params, imgs_iter, z, sigma_noise_iter
        )
    ...
    @jax.jit
    def proposal_func_orientations_uniform(
        key, angles0, logPiX0, v, shifts, ctf_params, imgs_iter, z
    ):
        empty_params = {}
        return proposal_func_orientations(
            key,
            angles0,
            logPiX0,
            v,
    ...
    @jax.jit
    def proposal_func_orientations_perturb(
        key, angles0, logPiX0, v, shifts, ctf_params, imgs_iter, z, sigma_perturb
    ):
        key, subkey = random.split(key)
    ...
    def proposal_func_shifts(key, shifts0, logPiX0, v, proj):
        ...
        def logPi(sh):
            sh_i = [
                -loss_proj_func_batched0_iter(
                    v, proj[i], sh[i], ctf_params[i], imgs_iter[i], sigma_noise_iter
                )
                for i in jnp.arange(shifts0.shape[0])
            ]
    ...
    def proposal_func_vol_batch(
        key, v0, logPiX0, angles, shifts, ctf_params, imgs_iter
    ):
        def logPi_vol(v):
            loss_i = [
                -loss_func_sum_iter(
        ...
        return proposal_hmc(
            key, v0, logPiX0, logPi_vol, gradLogPi_vol, dt_list_hmc, L_hmc, M_iter
        )

    @jax.jit
    def proposal_func_vol(key, v0, logPiX0, angles, shifts, ctf_params, imgs_iter):
        logPi_vol = lambda v: -loss_func_sum_iter(
            v, angles, shifts, ctf_params, imgs_iter, sigma_noise_iter
        )
    ...
    return (
        proposal_func_orientations_uniform,
        proposal_func_orientations_perturb,
        proposal_func_shifts,
        proposal_func_vol,
        proposal_func_vol_batch,
    )
...
def get_class_proposal_func(loss_func_batched, loss_func_sum, sigma_noise, alpha_d, K):
    ...
    def proposal_z_angles_batch(key, az0, logPiX0, v, shifts, ctf_params, imgs):
        ...
        logPiX0 = -loss_func_batched(
            v, angles0, shifts, ctf_params, imgs, z0, sigma_noise
        )
        logPiX1 = -loss_func_batched(
            v, angles1, shifts, ctf_params, imgs, z1, sigma_noise
        )
        r = jnp.exp(logPiX1 - logPiX0)
    ...
    def proposal_angles_z_mtm_batch(key, az0, logPiX0, v, shifts, ctf_params, imgs):
        ...
        wyk, angles1, z1, z1idx = multiple_try_metropolis_states(
            key1, angles0, N_class_samples, v, shifts, ctf_params, imgs
        )
        ...
        wxk = jax.vmap(
            lambda wyki, z1idxi, wxi: wyki.at[z1idxi].set(wxi), in_axes=(0, 0, 0)
        )(wyk, z1idx, wx)
    ...
    def multiple_try_metropolis_states(
        key, angles0, N_class_samples, v, shifts, ctf_params, imgs
    ):
        ...
        wk = jax.vmap(
            lambda zk: -loss_func_batched(
                v, angles1, shifts, ctf_params, imgs, zk, sigma_noise
            ),
            in_axes=(1),
        )(Krange).transpose()
        ...
        z1idx = jax.vmap(
            lambda keyi, wji: jax.random.categorical(keyi, wji), in_axes=(0, 0)
        )(keys, wj)
    ...
    return (
        ...
        proposal_z_angles_batch,
        proposal_angles_z_mtm_batch,
    )
...
def get_min_loss_indices(imgs1, imgs2, loss_func_array, sigma):
    return jax.vmap(get_min_loss_index, in_axes=(0, None, None, None))(
        imgs1, imgs2, loss_func_array, sigma
    )


def sample_new_angles_cached(
    loss_func_imgs_batched,
    slice_func_array_angles,
    vol,
    shifts_true,
    ctf_params,
    ...
    imgs_sampled = slice_func_array_angles(
        vol, ang_samples, shifts_true[0], ctf_params[0]
    )
    indices = get_min_loss_indices(
        imgs, imgs_sampled, loss_func_imgs_batched, sigma_noise
    )
...
def sample_new_angles_one_img(
    loss_func_angles, vol, shifts_true, ctf_params, img, N_samples, sigma_noise
):
    ang_samples = generate_uniform_orientations(N_samples)
...
def sample_new_angles(
    loss_func_angles, vol, shifts_true, ctf_params, imgs, N_samples, sigma_noise
):
    angles = []
    for ai in range(N):
        ang_samples = generate_uniform_orientations(N_samples)
        loss = loss_func_angles(
            vol, ang_samples, shifts_true[ai], ctf_params[ai], imgs[ai], sigma_noise
        )
        li = jnp.argmin(loss)
...
sample_new_angles_vmap = jax.vmap(
    sample_new_angles_one_img, in_axes=(None, None, 0, 0, 0, None, None)
)
...
def gradLogPi_split(
    ...
    grad_loss_vol = [
        grad_loss_volume_sum(
            v, angles_b[i], shifts_b[i], ctf_params_b[i], imgs_f_b[i], sigma_noise
        )
